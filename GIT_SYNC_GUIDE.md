...
icon: "🚀"                 # 分类图标（emoji）
sort: 1                    # 排序顺序（整数，默认 0）
post_sort: "published_at_desc" # 文章默认排序方式（可选）
---
...
#### `post_sort` / `post_sort_order` - 文章列表排序

- **类型**: 字符串 (枚举值)
- **必填**: 否
- **默认值**: `published_at_desc`
- **说明**: 定义该分类下文章列表的默认排序列
- **可选值**:
  - `published_at_desc`: 按发布时间**倒序**（最新在前）
  - `published_at_asc`: 按发布时间**正序**（最旧在前）
  - `title_asc`: 按标题 **A -> Z** 正序
  - `title_desc`: 按标题 **Z -> A** 倒序
- **示例**: `post_sort: "title_asc"`
...
| `post_sort`         | `post_sort_order` | 文章列表默认排序方式                           |
...
- ✅ 新增分类下文章列表自定义排序支持
- ✅ 支持在分类 `index.md` 中通过 `post_sort` 指定排序规则
- ✅ 优化 Alembic 迁移脚本的 Enum 检测逻辑
